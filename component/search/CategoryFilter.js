  ...
  return (
    ...
    <ScrollView style={{paddingHorizontal: 20}}>
        <View style={styles.categoryWrapper}>
            ...
            return(
            <TouchableOpacity onPress={()=>selectParentCategory(child._id)} key={child._id}>
                <Text style={selectedParentId == child._id ? styles.selectedName : styles.name }>{child.name}</Text>
            </TouchableOpacity>
            )
            })}
        </View>
        <View style={styles.childCategoryWrapper}>
            ...
            return(
            <TouchableOpacity key={child._id} onPress={()=>selectChildCategory(child._id)}>
                <Text style={selectedChildId == child._id ? styles.selectedName : styles.name}>{child.name}</Text>
            </TouchableOpacity>
            )
            })}
        </View>
        <View style={styles.grandChildCategoryWrapper}>
            ...
            return(
            <TouchableOpacity key={child._id} onPress={()=>selectGrandChildCategory(child)}>
                <Text style={selectedGrandChildId == child._id ? styles.selectedName : styles.name}>{child.name}</Text>
            </TouchableOpacity>
            )
            })}
        </View>
