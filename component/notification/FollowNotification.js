...
export default function FollowNotification(props) {
    ...
    useEffect(() => {
     if(item.follower.followers.includes(decode._id)){
         ...
         console.log(item.follower.followings,decode._id)
     }
    }, [])
    ...
    async function followBack(){
        setIsReady(false)
       try {
        const response = await axios.post('/user/follow/'+item.follower._id, {}, config)
        setIsReady(true)
        setIsFollowBack(true)
  ...
  return (
    ...
    {isReady?(
        isFollowBack?(
            <TouchableOpacity onPress={()=>messageLoader()} ><Text style={styles.message}>Message</Text></TouchableOpacity>
        ):(
            <TouchableOpacity onPress={()=>followBack()}><Text style={styles.followBack}>Follow</Text></TouchableOpacity>
        )
